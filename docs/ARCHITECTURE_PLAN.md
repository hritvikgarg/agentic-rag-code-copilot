...
- **STATUS:** Implemented and tested. The 6-configuration matrix (cap 512/768/1024 x prefixed/raw) has been run with the real model on Windows; results and limits are in `docs/evaluation.md`. **Decision: keep 512 + `prefixed` as the default dense configuration** (best Hit@1, Hit@3 and MRR on the pinned self-authored benchmark; `raw` had higher Hit@10; differences of 1-3 questions, no significance claimed; larger chunks have a line-overlap advantage). Cap 2048 was not run. Delivered: `VectorIndex.search` (deterministic ordering, FAISS `-1` filtering), `copilot.retrieval` (`Retriever`, `retrieve`, `RetrievalResult`, source re-materialisation with fingerprint and per-chunk hash verification, CLI), `copilot.evaluation` (JSONL benchmark with hash-pinned regions, Hit@k/MRR/mean-lines-per-hit, cap x representation matrix runner, CLI). Design in `docs/retrieval.md` and `docs/evaluation.md`.
...
- **GATE (mandatory, amendment 10; scanner delivered in Milestone 5c as `copilot.security.assert_safe_for_external_llm`):** the content-based secret scanner must exist, be tested and be wired in *before* any retrieved repository text is sent to a hosted LLM. `FakeLLM` and Ollama-local development may proceed without it; the first `GeminiClient` call with repository context may not.
...
15. **Default dense configuration decided by the Milestone 5b matrix (added after Windows verification).** Keep `chunk_max_tokens=512` with `embedding_text_style=prefixed`. Basis: best Hit@1 (47.1%), Hit@3 (70.6%) and MRR (0.603) of six configurations on a 34-question benchmark pinned to commit `055a8d5`, which is self-authored and not independent; `raw` reached higher Hit@10 (85.3% vs 82.4%) at every cap; differences are 1-3 questions and untested for significance. Larger chunks have a line-overlap advantage. The decision is revisited when an independent repository and structure-aware chunking exist.
16. **Content-secret scanner delivered (added at Milestone 5c).** `copilot.security` implements amendment 10's scanner and a fail-closed gate (`assert_safe_for_external_llm`, `RepositorySecretRiskError`). There is deliberately no bypass flag or allowlist. It is not yet wired to an LLM because none exists; wiring it immediately before any external call is a Milestone 6 acceptance criterion. It reduces risk and does not guarantee detection (`docs/security.md`).
